...
difference()
{
    ...
    // take out hoop
    #translate([0,-hoopRadius,-hoopTubeRadius+pThickness-pRecess]) scannerHoop();
    // make clamp over hoop corrugated (it won't curve with the torus though)
    // to try to increase clamping strength
    translate([0,-1.0,-hoopTubeRadius+pThickness-pRecess])
        for( i = [0:18:360] )
        {
    ...
    // hole for 1/4"-20 bolt to secure camera
    translate([0,50,-pThickness/2]) cylinder(r=0.25*25.4/2+0.3,h=pThickness*2,$fn=50);
}
